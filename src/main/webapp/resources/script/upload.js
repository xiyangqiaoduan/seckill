(function($){
	...
	$.extend($.fn,{
		upload:function(settings){
			var options=$.extends({
				fileType:"gif|jpg|jpeg|png|bmp",//允许上传的文件格式
				uploadUrl:"",//文件的上传地址
				deleteUrl:"",//文件的删除地址
				width:"",//图片的宽度
				height:100,//图片的高度
				imgSelector:'.imgDiv',//图片的选择器
				uploadData:{},//上传时需要附加的参数信息
			...
			var methods={
					...
					checkFile:function(fileName){
						var pos=fileName.lastIndexOf(".");//获取文件类型信息
						var str=fileName.substring(pos,fileName.length);
						var str1 = str.toLowerCase();  //获取文件类型名称，并转化为小写
						//判断传输的文件类型参数是否是字符串信息
